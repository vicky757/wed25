...
{
    public partial class NewFWUpgradation : Form
    {
        public NewFWUpgradation()
        {
            InitializeComponent();
        }
        ...
        {
            LogWriter.WriteToFile("NewFWUpgradation.cs - btn_Download_Click", "================================================ Upgrade Start ================================================", "RTC_Upgrade");
            pgbDownload.Value = 0;
            ...
            switch (res)
            {
                ...
                case 3:
                    if (SetValues.Set_SelectedPath != null)
                    {
                        ...
                        {
                            if (cmbBxDevice.SelectedIndex == 0)
                                ClassList.CommonConstants.SetFileType = 1;
                            else if (cmbBxDevice.SelectedIndex == 1)
                                ClassList.CommonConstants.SetFileType = 2;
                            else if (cmbBxDevice.SelectedIndex == 2)
                                ClassList.CommonConstants.SetFileType = 3;
                            else
                                ClassList.CommonConstants.SetFileType = 4;

                            SetValues.Set_NodeAddress = Convert.ToByte(numericUpDown1.Text);
        ...
        private void cmbBxDevice_SelectedIndexChanged(object sender, EventArgs e)
        {
           // groupBox01.Visible = true;
           // groupBox02.Visible = true;
           /* if (cmbBxDevice.SelectedIndex == 0)
            {
                groupBox02.Visible = false;
                groupBox01.Visible = true;
                numericUpDown1.Value = 1;
    
                
            }
            if (cmbBxDevice.SelectedIndex == 1)
            {
                groupBox01.Visible = false;
                groupBox02.Visible = true;
        ...
        private void NewFWUpgradation_Load(object sender, EventArgs e)
        {
        ...
        public int CheckFileLoc()
        {
            ...
            if  (cmbBxDevice.SelectedIndex == 0)
                ...
                logFilePath = m_exePath1 + "\\HIO\\Object\\FL002-0102TR";
            else
                logFilePath = m_exePath1 + "\\HIO\\Object\\FLA0102TR";

            if (!string.IsNullOrEmpty(logFilePath))
            {

                DirectoryInfo logDirInfo1 = new DirectoryInfo(logFilePath);
                ...
                if (logDirInfo1.Exists)
                {
                    ...
                    {
                        DirectoryInfo logDirInfo2 = new DirectoryInfo(logFilePath1);
                        if (logDirInfo2.Exists)
                        {
                            int numABS = logDirInfo2.GetFiles("*.abs", SearchOption.AllDirectories).Length;
        ...
        private void GetDownLoadParameterOnSerial(ProgressBar pProgressBar, StatusStrip pStatusBar, Label pProgressLabel)
        {
            try
            {
                LogWriter.WriteToFile("NewFWUpgradation.cs - GetDownLoadParameterOnSerial()", "started--", "RTC_Upgrade");
                Refresh();
        ...
        public void DownloadOnSerial_Update1(ClassList.DownloadData pDownloadData, string pFileName, int pProductID, string pComNoOrIpAddress, byte[] _setupFrameMode)
        {
            try
            {
                LogWriter.WriteToFile("NewFWUpgradation.cs - DownloadOnSerial_Update1()", "started--", "RTC_Upgrade");
                ...
                serial__deviceDownloadStatus(105);
                {
                    if (serial.Connect() == ClassList.CommonConstants.SUCCESS)
                    {
                        LogWriter.WriteToFile("NewFWUpgradation.cs-", "Start", "RTC_Upgrade");
        ...
        private bool SendOnDevice(string downloadAddress)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string nodeAddress = Convert.ToString(numericUpDown1.Value);
